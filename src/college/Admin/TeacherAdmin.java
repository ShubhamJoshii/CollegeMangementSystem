...
public final class TeacherAdmin extends javax.swing.JPanel {
    MainFrame main;
    DefaultTableModel model;

    public TeacherAdmin(MainFrame main) {
        initComponents();
        this.main = main;
    ...
    public void fetchCourses() {
        try {
            ...
            while (rs.next()) {
                ...
                selectCourse.addItem(departmentId + " - " + departmentName);
            }
        } catch (ClassNotFoundException | SQLException e) {
        }
    }

    public void fetchStudents(String selectedDepartment) {
        String departmentId = selectedDepartment.split("-")[0].trim();
        String departmentName = selectedDepartment.split("-")[1].trim();
        ...
        try {
            ...
            String sql
                    = "SELECT u.userId, u.userName, u.email, u.contact, u.status "
                    + "FROM users u "
                    + "JOIN teachers t ON u.userId = t.userId "
                    + "JOIN departments d ON t.departmentId = d.departmentId "
                    + "WHERE d.departmentId = ? AND u.role = ?";
            ...
            while (rs.next()) {
                ...
                model.addRow(new Object[]{++count, id, userName, email, contactNumber, departmentName,  userStatus});
            }
        } catch (ClassNotFoundException | SQLException e) {
    ...
    public void fetchStudents(String selectedDepartment, String Name) {
        String departmentId = selectedDepartment.split("-")[0].trim();
        String departmentName = selectedDepartment.split("-")[1].trim();
        try {
            Connection conn = DBConnection.getConnection();
            ...
            String sql
                    ...
                    + "JOIN teachers t ON u.userId = t.userId "
                    + "JOIN departments d ON t.departmentId = d.departmentId "
                    + "WHERE d.departmentId = ? AND (u.userName LIKE ? OR u.userId LIKE ?)";
    ...
    private void initComponents() {
        ...
        studentTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "S. No.", "Registration No.", "Full Name", "Email", "Contact Number", "Course", "Profile Complete"
            }
        ) {
    ...
    private void selectCourseActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_selectCourseActionPerformed
        ...
        if (selected != null) {
            fetchStudents(selected.toString());
        }
...
//        WHERE c.courseName = 'MCA';
    ...
    private void searchTextBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchTextBtnActionPerformed
        Object selected = selectCourse.getSelectedItem();
        if (selected != null) {
            fetchStudents(selected.toString(), searchText.getText());
        }
    }//GEN-LAST:event_searchTextBtnActionPerformed

    private void SavebtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SavebtnActionPerformed
        AddTeacherThroughAdmin addTeacherThroughAdmin;
        addTeacherThroughAdmin = new AddTeacherThroughAdmin(null, true);
        addTeacherThroughAdmin.setVisible(true);
